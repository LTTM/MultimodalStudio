...
from cameras import camera_utils
...
class Cameras(TensorDataclass):
    ...
    def _generate_rays_from_coords(
        ...
        up_directions = torch.tensor([0.0, 1.0, 0.0], device=self.device).expand_as(directions)
        up_directions = c2w[..., :3] @ up_directions[..., None]
        up_directions = up_directions.squeeze()
        assert up_directions.shape == num_rays_shape + (3,)
    ...
    def get_c2w_matrices(self, indexes):
        """Returns the camera-to-world matrices for the cameras identified by the given indexes."""
        return self.camera_to_worlds[indexes]
