...
from utils.decorators import check_main_thread, decorate_all
from utils.printing import human_format
...
@check_main_thread
def write_out_storage(step, steps_per_flush_buffer: int = 10):
    ...
    for writer in EVENT_WRITERS:
        if isinstance(writer, LocalWriter) and len(EVENT_STORAGE) > 0:
            writer.write_stats_log(step)
            continue
        if step % steps_per_flush_buffer == 0 and step > 0:
...
def setup_event_writer(config: cfg.Config, log_dir: Path) -> None:
    ...
    if config.is_wandb_enabled():
        curr_writer = WandbWriter(log_dir=log_dir, experiment_name=config.method_name)
        EVENT_WRITERS.append(curr_writer)
    ...
    def write_scalar_dict(self, name: str, scalar_dict: Dict[str, Any], step: int) -> None:
        ...
        for key, value in scalar_dict.items():
            if isinstance(value, dict):
                for key2, scalar in value.items():
                    scalar = float(scalar) if not isinstance(scalar, str) else np.nan
                    self.write_scalar(name + "/" + key + "_" + key2, scalar, step)
            else:
                self.write_scalar(name + "/" + key, float(value), step)
...
class WandbWriter(Writer):
    ...
    def __init__(self, log_dir: Path, experiment_name: str):
        wandb.init(project="MultimodalStudio", name=experiment_name, dir=str(log_dir), reinit=True)
...
class LocalWriter:
    ...
    def write_config(self, name: str, config_dict: Dict[str, Any], step: int):
        ...
        Args:
            config: config dictionary to write out
        """
        # (SDFSStudio comment): implement this
