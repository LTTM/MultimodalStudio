...
class TensorDataclass:
    ...
    def detach(self: TensorDataclassT) -> TensorDataclassT:
        """Returns a new TensorDataclass with the same data but detached.

        Returns:
            A new TensorDataclass with the same data but detached.
        """
        return self._apply_fn_to_fields(lambda x: x.detach())

    def flip(self: TensorDataclassT, dims: Union[int, List[int], Tuple[int]]) -> TensorDataclassT:
        """Returns a new TensorDataclass with the same data but with the specified dimensions flipped.

        Args:
            dims: The dimensions to flip.

        Returns:
            A new TensorDataclass with the same data but with the specified dimensions flipped.
        """
        return self._apply_fn_to_fields(lambda x: torch.flip(x, dims))
