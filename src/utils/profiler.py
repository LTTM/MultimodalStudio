...
from configs.configs import LoggingConfig
from utils import comms
from utils.decorators import check_main_thread, check_profiler_enabled, decorate_all
...
def setup_profiler(config: LoggingConfig, log_dir: Union[Path, str]):
...
class PytorchProfiler:
    ...
    def __init__(self, output_path: Union[Path, str], trace_steps: Optional[List[int]] = None):
        self.output_path = os.path.join(output_path, "profiler_traces")
    ...
    def record_function(self, function: str, *args, **_kwargs):
        ...
        """
        if function.endswith("train_step") or function.endswith("eval_step"):
            step = args[1]
            ...
            if step in self.trace_steps:
                ...
                prof.export_chrome_trace(
                    os.path.join(
                        self.output_path, f"trace_{stage}_{step}{'_blocking' if launch_kernel_blocking else ''}.json"
                    )
                )
                return
